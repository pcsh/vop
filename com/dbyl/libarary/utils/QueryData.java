...
public class QueryData {
	...
	public Boolean OnlyouUserExist(String url,String user,String pwd,String UserName,String Mobile,String Email) throws SQLException{
		String UID = null;
		try{
			Class.forName("org.postgresql.Driver");
		}catch(ClassNotFoundException e){
		...
		if (connection != null) {
			//System.out.println("You made it, take control your database now!");
	          Statement st = (Statement) connection.createStatement();
	           String sql = "select name from t_onlyou_user where name = '"+UserName+"' ORDER BY create_ts DESC limit 1";
	        		   //"select ordernum from t_order where customerid in(select tc.customerid from t_customer tc where tc.userid in (select t.userid from t_user t where t.mpnumber = '"+mpnumber+"')) ORDER BY createtime DESC limit 1" ;
	           ResultSet rs = ((java.sql.Statement) st).executeQuery(sql);
	            while (rs.next())
	            {
		           ...
		           String sql2 = "select name from t_onlyou_user where mobile_phone_no = '"+Mobile+"' ORDER BY create_ts DESC limit 1";
        		   //"select ordernum from t_order where customerid in(select tc.customerid from t_customer tc where tc.userid in (select t.userid from t_user t where t.mpnumber = '"+mpnumber+"')) ORDER BY createtime DESC limit 1" ;
           ResultSet rs2 = ((java.sql.Statement) st).executeQuery(sql2);
            while (rs2.next())
            {
	           ...
	           String sql3 = "select name from t_onlyou_user where email = '"+Email+"' ORDER BY create_ts DESC limit 1";
 		   //"select ordernum from t_order where customerid in(select tc.customerid from t_customer tc where tc.userid in (select t.userid from t_user t where t.mpnumber = '"+mpnumber+"')) ORDER BY createtime DESC limit 1" ;
    ResultSet rs3 = ((java.sql.Statement) st).executeQuery(sql3);
     while (rs3.next())
     {
